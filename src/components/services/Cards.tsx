  ...
  return (
    <>
      <div className={`flex flex-col gap-4 items-start p-5 border ${borderColor} rounded-3xl mt-10 h-[22.5rem]`}>
        <img className=" w-28 services-img-xlbig:w-32" src={icon} alt="Icono de la tarjeta" />
        <h1 className={`${textColor} text-2xl`}>
          {titleOne}<span className={positionColor}>{titleTwo}</span>
        </h1>
        <p className="font-quicksand font-extralight text-[1.2rem] services-text-big:text-[1.3rem] w-[95%] services-text-xbig:text-2xl services-text-xbig:w-[90%]">
          {description}
        </p>
